...
class BaseRover(ABC, PushMixin):
    def __init__(self, config):
        self.config = config
        self.device = self.config.device
        self.seed = self.config.Rover.seed
        set_seed(self.seed)
        self.setup_logging()
    ...
    def initialize_deap(self):
        ...
        self.toolbox.decorate(
            "mate",
            gp.staticLimit(
                operator.attrgetter("height"), self.config.Rover.gp_tree_height
            ),
        )
    ...
    def create_n_evaluate(self, individual, datasets):
        fitness = 0.0
        for dataset in datasets:
            for architecture in self.config.Rover.architectures[dataset.name]:
                ...
                try:
                    self.train(model, train_loader=dataset.train_loader)
                    fitness += (
                        self.evaluate(model, val_loader=dataset.val_loader) * dataset.weight * self.config.Rover.architectures_weights[architecture]
                    )
                except Exception as e:
                    logging.error(e)
                    return (0.0,)
    ...
    def mine(self):
        ...
        datasets = load_datasets(
            self.config.Rover.architectures.keys(), 
            batch_size=self.config.Rover.batch_size,
            seed=self.config.Rover.seed
        )
        ...
        if os.path.exists(checkpoint_file):
            ...
            population = self.toolbox.population(n=self.config.Rover.population_size)
        ...
        for generation in tqdm(range(start_generation, self.config.Rover.generations)):
            ...
            for i in range(0, len(offspring), 2):
                if random.random() < 0.5:
                    if i + 1 < len(offspring):
                        ...
                        if child1.height > self.config.Rover.gp_tree_height:
                            offspring[i] = safe_temp1

                        if child2.height > self.config.Rover.gp_tree_height:
                        ...
                        for i in range(len(offspring)):
                            if random.random() < 0.2:
                                ...
                                if mutant.height <= self.config.Rover.gp_tree_height:
                                    offspring[i] = mutant
                                    del offspring[i].fitness.values
            ...
            if generation % self.config.Rover.check_registration_interval == 0:
    ...
    @staticmethod
    def setup_logging(log_file="rover.log"):
        logging.basicConfig(
            filename=log_file,
            level=logging.INFO,
...
class BaseHuggingFaceRover(BaseRover):
    def __init__(self, config):
...
class BaseMiningPoolRover(BaseRover):
    def __init__(self, config):
        super().__init__(config)
        self.push_destinations.append(
            PoolPushDestination(config.Rover.pool_url, config.bittensor_network.wallet)
        )
        self.pool_url = config.Rover.pool_url
    ...
    def update_config_with_task(self, task):
        # Update rover config with task-specific parameters if needed
        pass


class IslandRover(BaseRover):
    def __init__(self, config):
        super().__init__(config)
        self.num_islands = config.Rover.num_processes
        self.migration_interval = config.Rover.migration_interval
        self.population_per_island = config.Rover.population_size // self.num_islands
        self.migrants_per_round = getattr(
            config.Rover, "migrants_per_round", 1
    ...
    def run_island(
        ...
        datasets = load_datasets(self.config.Rover.architectures.keys())

        while generation < self.config.Rover.generations:
            # Evolution step
            if self._shutdown:
                break
...
class ActivationRover(BaseRover):
    ...
    def train(self, model, train_loader):
        ...
        for idx, (inputs, targets) in enumerate(train_loader):
            ...
            if idx == self.config.Rover.training_iterations:
                break
            optimizer.zero_grad()
    ...
    def evaluate(self, model, val_loader):
        ...
        with torch.no_grad():
            for idx, (inputs, targets) in enumerate(val_loader):
                inputs = inputs.to(self.device)
                targets = targets.to(self.device)
                if idx >= self.config.Rover.evaluation_iterations:
...
class SimpleRover(BaseRover):
    ...
    def mine(self):
        ...
        population, logbook = algorithms.eaSimple(
            self.toolbox.population(n=self.config.Rover.population_size),
            self.toolbox,
            cxpb=0.5,
            mutpb=0.2,
            ngen=self.config.Rover.generations,
            stats=self.stats,
            halloffame=self.hof,
            verbose=True,
        )
...
class OptimizerRover(BaseRover):
    def __init__(self, config):
        super().__init__(config)
...
class ParallelOptimizerRover(BaseRover):
    def __init__(self):
...
class ActivationRoverHF(ActivationRover, BaseHuggingFaceRover):
    pass


class ParallelActivationRover(ActivationRover, IslandRover):
    pass
...
class ParallelActivationRoverHF(ParallelActivationRover, BaseHuggingFaceRover):
    pass


class ParallelLossRover(LossRover, IslandRover):
    pass
...
class OptimizerRoverHF(OptimizerRover, BaseHuggingFaceRover):
    pass
...
class ParallelLossRoverHF(ParallelLossRover, BaseHuggingFaceRover):
    pass
...
class ParallelOptimizerRoverHF(ParallelOptimizerRover, BaseHuggingFaceRover):
    pass
...
class SimpleRoverHF(SimpleRover, BaseHuggingFaceRover):
    pass


class RoverFactory:
    @staticmethod
    def get_rover(config):
        rover_type = config.Rover.rover_type
        platform = config.Rover.push_platform
        core_count = config.Rover.num_processes
        if platform == "pool":
            if rover_type == "activation":
                if core_count == 1:
                    return ActivationRoverPool(config)
                else:
                    return ParallelActivationRoverPool(config)
            elif rover_type == "loss":
                if core_count == 1:
                    return LossRoverPool(config)
                else:
                    return ParallelLossRoverPool(config)
            elif rover_type == "optimizer":
                if core_count == 1:
                    return OptimizerRoverPool(config)
                else:
                    return ParallelOptimizerRoverPool(config)
        elif platform == "hf":
            if rover_type == "activation":
                if core_count == 1:
                    return ActivationRoverHF(config)
                else:
                    return ParallelActivationRoverHF(config)
            elif rover_type == "loss":
                if core_count == 1:
                    return LossRoverHF(config)
                else:
                    return ParallelLossRoverHF(config)
            elif rover_type == "optimizer":
                if core_count == 1:
                    return OptimizerRoverHF(config)
                else:
                    return ParallelOptimizerRoverHF(config)

        raise ValueError(f"Unknown rover type: {rover_type} or platform: {platform}")
