...
from dml.configs.auditor_config import constrained_decay
...
class BaseAuditor(ABC):
    def __init__(self, config):
        ...
        self.interval = config.Auditor.validation_interval
        self.gene_record_manager = GeneRecordManager()
        self.scores = {}
        self.normalized_scores = {}
        self.metrics_file = config.metrics_file
        self.metrics_data = []
        self.seed = self.config.Auditor.seed

        self.chain_metadata_cache = {}

        self.penalty_factor = config.Auditor.time_penalty_factor
        self.penalty_max_time = config.Auditor.time_penalty_max_time
        ...
        self.cache_interval = config.Auditor.cache_interval
    ...
    def evaluate_individual(self, individual, datasets):
        try:
            ...
            for dataset in datasets:
                for architecture in self.config.Auditor.architectures[dataset.name]:
    ...
    def compute_ranks(self, filtered_scores_dict):
        ...
        # Convert dict to tensor matrix [n_rovers x n_datasets]
        try:
        ...
        # Average rank across datasets for each rover
    ...
    def calculate_topk_scores(self, score_dict: Dict[str, float], all_hotkeys: list) -> Dict[str, float]:
        """
        Calculate normalized top-k scores for rovers with dynamic weight redistribution.
        When there are ties, remaining weights are pooled and distributed equally among
        all rovers with the same score.
        """
        ...
        if filtered_scores:
            ...
            top_k_weights = self.config.Auditor.top_k_weight
    ...
    def validate_and_score(self):
        ...
        #     logging.info("This auditor is no longer registered on the chain.")
        #     return
        ...
        datasets = load_datasets(self.config.Auditor.architectures.keys(), batch_size=32)

        temp_scores = {h: 0.0 for h in self.bittensor_network.metagraph.hotkeys}
        validated_rovers = set()
                    ...
                    if func_signature:
                        if func_signature in round_signatures:
                            ...
                            if existing_record["block_number"] < existing_block:
                                ...
                                if hotkey_address not in validated_rovers:
                                    validated_rovers.add(hotkey_address)
        ...
        for hotkey_address in tqdm(self.bittensor_network.metagraph.hotkeys):
            if (
                ...
                if self.validation_round_count < self.config.Auditor.caching_rounds:
                    current_time = time.time()
                    ...
                    if func_signature in round_signatures:
                        ...
                        if (
                            ...
                            try:
                                ...
                                logging.info(f"Evaluation failed for {hotkey_address}. Assigning 0 score. Rover Caused Error: {e}")
                                accuracy_scores[hotkey_address] = torch.tensor(
        ...
        try:
            ...
            if "best_gene.json" in file_info:
                ...
                if file_details:
                    ...
                    max_size = self.config.Auditor.max_gene_size
...
class ActivationAuditor(BaseAuditor):
    def load_data(self):
...
class LossAuditor(BaseAuditor):
    ...
    def evaluate(self, model_and_loss, val_loader=None):
        try:
            ...
            checkpoints = [
                int(self.config.Auditor.training_iterations * x) 
            ...
            for idx, (inputs, targets) in enumerate(train_dataloader):
                if idx == self.config.Auditor.training_iterations:
                    break
    ...
    def evaluate_validation_set(self, model, val_dataloader):
        ...
        with torch.no_grad():
            for idx, (inputs, targets) in enumerate(val_dataloader):
                if idx > self.config.Auditor.validation_iterations:
                    break
                inputs = inputs.to(self.device)
                targets = targets.to(self.device)
                outputs = model(inputs)
...
class LossAuditorEnhanced(LossAuditor):
    def __init__(self, config):
        super().__init__(config)
        self.architectures = self._setup_architectures()
        self.datasets = load_datasets(config.Auditor.architectures.keys())
...
class OptimizerAuditor(BaseAuditor):
    def __init__(self, config):
        super().__init__(config)
        self.baseline_times = {}
    ...
    def evaluate_individual(self, individual, datasets):
        ...
        for dataset in datasets:
            ...
            try:
                ...
                start_time = time.time()
                model.train()
                for epoch in range(self.config.Auditor.training_epochs):
...
class AuditorFactory:
    @staticmethod
    def get_auditor(config):
        auditor_type = config.Auditor.auditor_type
        if auditor_type == "activation":
            return ActivationAuditor(config)
        elif auditor_type == "loss":
            return LossAuditor(config)
        elif auditor_type == "optimizer":
            return OptimizerAuditor(config)
        else:
            raise ValueError(f"Unknown auditor type: {auditor_type}")
